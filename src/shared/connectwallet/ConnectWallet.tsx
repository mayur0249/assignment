  ...
  const getBalance = () => {
    if (account && library) {
      ...
      library.eth
        ...
        .then((balance: any) => {
          // if (!stale) {
          //   const Temp = parseFloat(formatEther(balance)).toPrecision(4);
          // }
          let bal = new BigNumber(balance).dividedBy(10 ** 18).toFixed(3);
          setBalance(bal);
        ...
        {showWalletContent ? (
          connectWallet ? (
            <AddressInfoWrap menu={props.menu}>
              <BnbInfo menu={props.menu}> {balance} ETH</BnbInfo>
              <AddressInfo
                onClick={() => {
